...
const CartPage = () => {
  ...
  return (
    <div>
      ...
      {/* <div className="max-w-full mx-auto bg-white p-8 mt-8 pt-16">
        <h2 className="font-bold mb-4 text-center font-extrabold text-2xl p-2 shadow-xl bg-gradient-to-tr from-yellow-400 to-red-400">Cart</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 p-4">
          {cartItems.map((product) => (
            <div key={product.id} className="border rounded-lg p-4 bg-white shadow-xl">
              <img src={product.image} alt={product.name} className="w-full h-64 object-cover mb-4 rounded-lg" />
              <h2 className="font-bold text-xl mb-2">{product.name}</h2>
              <p className="text-gray-700 mb-2">Winning Bid: {product.price} ฿</p>
            </div>
          ))}
        </div>
    </div> */}
